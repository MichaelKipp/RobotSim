...
public class TartGene extends GPGene {
    ...
    int evaluate(TartVariables cfg, TartGP gp) {
        ...
        // Terminal that returns random between 0 and 2
        case Grid.RND:
            return ThreadLocalRandom.current().nextInt(0, 2 + 1);

        // Function to look two spaces ahead
        case Grid.TWOAHEAD:
            int move = cfg.dozerGrid.sensor(2, 0);
            if (move == 0)
              return ( (TartGene)get(0)).evaluate(cfg, gp);
            else if (move == 1)
              return ( (TartGene)get(1)).evaluate(cfg, gp);
            else
              return ( (TartGene)get(2)).evaluate(cfg, gp);

        // Function to look three spaces ahead
        case Grid.THREEAHEAD:
            move = cfg.dozerGrid.sensor(3, 0);
            if (move == 0)
              return ( (TartGene)get(0)).evaluate(cfg, gp);
            else if (move == 1)
              return ( (TartGene)get(1)).evaluate(cfg, gp);
