...
public class Grid {
    ...
    private void initGrid() {
        ...
        // place dozer at start location(bottom gate)
        x = rgen.nextInt(xdim-xdim/2) + xdim/2 - 1;
        y = ydim - 1;
        gridEndGate = rgen.nextInt(3);

        // Prevents robot from being place in a corner
        if (grid[x][y] == 'X') {
            if (grid[x+1][y] != 'X') x++;
   ...
   // if no out file specified, use null
    ...
    // determine the fitness of the current state of the grid. fitness is 20 - (steps * .1) - Distance from exit
    public double calcFitness(int steps) {
    ...
    public static void main (String[] args) {
      ...
      for (int i = 0; i < numBoards; i++){
          ...
          for (int j = 0; j < numMoves; j++) {
              ...
              grid.stepsTaken++;
              if (grid.dozerX == grid.exitX && grid.dozerY == grid.exitY){
                break;
              }
          }
          System.out.println("Board: " + i + " fitness: " + curFitness);
          if (curFitness > bestFitness) {
            bestFitness = curFitness;
            bestBoardIndex = i;
          }
          //calculate fitness of board and add to averageFitness
          averageFitness += curFitness;

      }
      averageFitness = averageFitness/numBoards;
      System.out.println("Over " + numMoves + " moves and "
                        + numBoards + " boards the average fitness was: " + averageFitness
                        + " and the best fitness was: " + bestFitness + " on board " + bestBoardIndex);
